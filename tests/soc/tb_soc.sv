...
module tb_soc (
  ...
  assign periph_rdata = 32'd0;   // no readable peripherals needed for this test
  ...
  // ── Finish hook: a store to 0x8000_1000 ends the sim and prints the result ──
  always_ff @(posedge clk) begin
    if (periph_we && (periph_addr == 32'h8000_1000)) begin
      $display("SoC SDRAM test Finished. Result: %h", periph_wdata);
      $finish;
    end
  end
